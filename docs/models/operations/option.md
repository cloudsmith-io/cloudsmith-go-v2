...
```go
cloudsmith.WithServerURL("https://api.example.com")
```
...
```go
cloudsmith.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
cloudsmith.WithClient(httpClient)
```
...
```go
cloudsmith.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
cloudsmith.WithTimeout(30 * time.Second)
```
